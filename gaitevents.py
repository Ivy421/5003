...
class GaitData:
    ...
    # JSLOW(20231016): disable world unit to use scaled unit based on leg length measured
    ...
    # adjustOpenpose: 0 for raw, 1 for OPA, 2 for RJN, 3 for Mediapipe, 4 for manual, 5 for Mediapipe OPA, 6 for Mediapipe RJN
...
def openposeAdjustment(keypoints, fps, unit):
...
def front_analysis(gaitdata):
    ...
    mid_toe_raw = {Side.LEFT: np.array(data[Joint.LEFT_TOES] - data[Joint.MIDDLE_HIP]), 
                    Side.RIGHT: np.array(data[Joint.RIGHT_TOES] - data[Joint.MIDDLE_HIP])}
    
    #JSLOW(20230630) - adjust keypoints detected by Openpose
    if hasattr(gaitdata,"adjustOpenpose"):
        if gaitdata.adjustOpenpose == 1:
            adjustedKeypoints = openposeAdjustment([heel_raw, toe_raw, ankle_raw, knee_raw, hip_raw], gaitdata.fps, 'P')
        ...
        if gaitdata.adjustOpenpose == 5:
            adjustedKeypoints = openposeAdjustment([heel_raw, toe_raw, ankle_raw, knee_raw, hip_raw], gaitdata.fps, 'M')
            heel_raw, toe_raw = adjustedKeypoints['heel'], adjustedKeypoints['toe']
            ankle_raw, knee_raw, hip_raw = adjustedKeypoints['ankle'], adjustedKeypoints['knee'], adjustedKeypoints['hip']

    padlength = 5 
    b, a = scipy.signal.butter(3, 7.0, 'lp', fs=gaitdata.fps, output='ba')
    ankle_filt, knee_filt, hip_filt, heel_filt, toe_filt, mid_toe_filt = {}, {}, {}, {}, {}, {}
    for side in Side:
        ...
        mid_toe_filt[side] = scipy.signal.filtfilt(b, a, mid_toe_raw[side], axis=0, padlen=padlength)
    ...
    hip_adduction, knee_adduction, heel_distance, step_width = {}, {}, {}, {}
    for side in Side:
        ...
        step_width[side] = np.absolute(heel_filt[Side.LEFT][:,0]+mid_toe_filt[Side.LEFT][:,0]-heel_filt[Side.RIGHT][:,0]-mid_toe_filt[Side.RIGHT][:,0])/2
    ...
    kinedata = {
        ...
        'internal step width': {Side.LEFT:step_width[Side.LEFT], Side.RIGHT:step_width[Side.RIGHT]}
    }
...
def cbta(gaitdata , filename):
    ...
    if hasattr(gaitdata,"adjustOpenpose"):
        if gaitdata.adjustOpenpose == 2 or gaitdata.adjustOpenpose == 6:
    ...
    if hasattr(gaitdata,"adjustOpenpose"):
        if gaitdata.adjustOpenpose == 1:
            adjustedKeypoints = openposeAdjustment([heel_raw, toe_raw], gaitdata.fps, 'P')
            heel_raw, toe_raw = adjustedKeypoints['heel'], adjustedKeypoints['toe']
        if gaitdata.adjustOpenpose == 5:
            adjustedKeypoints = openposeAdjustment([heel_raw, toe_raw], gaitdata.fps, 'M')
            heel_raw, toe_raw = adjustedKeypoints['heel'], adjustedKeypoints['toe']
...
def print_analysis_HSTO(HS, TO, use_gaitdata: bool, gaitdata = None, fps=50, kinedata={} , filename='none'):
    ...
    double_supports = {Side.LEFT: [], Side.RIGHT: []}
    foot_offs = {Side.LEFT: [], Side.RIGHT: []}
    limp_indexes = {Side.LEFT: [], Side.RIGHT: []}
    opp_foot_contacts = {Side.LEFT: [], Side.RIGHT: []}
    opp_foot_offs = {Side.LEFT: [], Side.RIGHT: []}
    single_supports = {Side.LEFT: [], Side.RIGHT: []}
    ...
    for side in Side:
        ...
        for i in range(len(stride_pairs[side])):
            ...
            double_supports[side].append(double_support)
            ...
            foot_offs[side].append(foot_off)
            ...
            limp_indexes[side].append(limp_index)
            ...
            for k in range(len(HS[side.opposite()])):
                ...
                if HS[side.opposite()][k] >= stride_pairs[side][i][0] and HS[side.opposite()][k] <= stride_pairs[side][i][1]:
                    ...
                    break
            opp_foot_contacts[side].append(opp_foot_contact)
            ...
            for k in range(len(TO[side.opposite()])):
                ...
                if TO[side.opposite()][k] >= stride_pairs[side][i][0] and TO[side.opposite()][k] <= stride_pairs[side][i][1]:
                    ...
                    break
            opp_foot_offs[side].append(opp_foot_off)
            ...
            single_supports[side].append(single_support_time)
    ...
    for side in Side:
        ...
        for i in range(len(stride_pairs[side])):
            csvwriter.writerow(['Double support (s)', str(double_supports[side][i])])
            csvwriter.writerow(['Foot off (%)', str(foot_offs[side][i])])
            csvwriter.writerow(['Limp index', str(limp_indexes[side][i])])
            csvwriter.writerow(['Opposite foot contact (%)', str(opp_foot_contacts[side][i])])
            csvwriter.writerow(['Opposite foot off (%)', str(opp_foot_offs[side][i])])
            csvwriter.writerow(['Single support (s)', str(single_supports[side][i])])
            ...
            # get step width for front analysis at HS or initial double support time
            if 'internal step width' in kinedata:
                step_width = kinedata['internal step width'][side][stride_pairs[side][i][0]] * metres_per_pixel[side][stride_pairs[side][i][0]]
                all_hs = np.array(stride_pairs[side])[:,0]
                step_width_variability = np.std(kinedata['internal step width'][side][all_hs] * np.array(metres_per_pixel[side])[all_hs])/np.mean(kinedata['internal step width'][side][all_hs] * np.array(metres_per_pixel[side])[all_hs])*100
                csvwriter.writerow(['Step width (m)', str(step_width)])
                csvwriter.writerow(['Variabillty(step width) (%)', str(step_width_variability)])
            ...
            #JSLOW(20230911) - add variability (coefficient of variation = SD / mean x 100)
            #JSLOW(20231024) - moving average window of 3 strides (before, current, after)
            if i>0 and i+1 < len(stride_pairs[side]):
                if use_gaitdata:
                    csvwriter.writerow(['Variabillty(stride length) (%)', str(np.std(stride_lengths[side][i-1:i+2])/np.mean(stride_lengths[side][i-1:i+2])*100)])
                csvwriter.writerow(['Variabillty(stride time) (%)', str(np.std(stride_durations[side][i-1:i+2])/np.mean(stride_durations[side][i-1:i+2])*100)])
                stride_time_asymmetry = (np.mean(stride_durations[side][i-1:i+2])-np.mean(stride_durations[side.opposite()][i-1:i+2]))/(np.mean(stride_durations[Side.LEFT][i-1:i+2])+np.mean(stride_durations[Side.RIGHT][i-1:i+2]))*200
                csvwriter.writerow(['Asymmetry(stride time) (%)', str(stride_time_asymmetry)])
...
# JSLOW(20230216) - add csv output for kinematic data per frame per side
    ...
    for side in Side:
        ...
        for kineType, kineAngles in kinedata.items():
            # 'internal*' are not for output
            if not kineType.startswith('internal'):
