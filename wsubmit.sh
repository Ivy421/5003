...
MPLOGFILE=$OPENPOSE_ROOT/log/${videoName}_mp-`date +%Y%m%d%H%M%S`.log
echo singularity exec --nv -bind=/data $OPENPOSE_ROOT/opp20.sif $OPENPOSE_ROOT/gait/wrun.sh "$submitName" $2 $3 $4
echo $LOGFILE
singularity exec --nv --bind=/data $OPENPOSE_ROOT/mp.sif $OPENPOSE_ROOT/gait/wrun_mediapipe.sh "$submitName" $2 $3 >& "$MPLOGFILE" &
